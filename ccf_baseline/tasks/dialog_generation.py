...
class DialogGeneration(Task):
    ...
    def add_cmdline_args(cls, parser):
        ...
        group.add_argument("--is_cn", type=str2bool, default=True)
    ...
    def __init__(self, args):
        ...
        self.output_all_candidates = False
        return

    def debug(self):
        self.output_all_candidates = True
        return
    ...
    def _post_process_generation_output(self, predictions):
        ...
        for data_id in group:
            ...
            preds = group[data_id]
            #self.reader.tokenizer.tokenize(example.src)
            for pred in preds:
                #tokens = [self.reader.tokenizer.tokenize(s) for s in example.src.split("[SEP]")]
                tokens = post_process_context(pred["context_token_ids"], self.reader)
                pred_token_ids, pred_tokens = post_process_response(pred["response_token_ids"], self.reader)
                num_token = len(pred_token_ids)

                cross_turn_repetition = get_cross_turn_repetition(
                    tokens, pred_tokens, self.reader.eos_id, self.is_cn)
                
                in_turn_repetition = get_in_turn_repetition(pred_tokens, self.is_cn) \
                    or get_in_turn_repetition(pred_token_ids)

                pred["response"] = " ".join(pred_tokens)
